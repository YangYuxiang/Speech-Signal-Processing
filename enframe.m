...
nwin=length(win);
if nwin == 1        % win is the window length in samples, so nwin == 1
    lw = win;       % lw also refers to window length
    w = ones(1,lw); % w is  a row vector full of ones
else
    lw = nwin;      % win is the window, so nwin refers to window length, i.e. lw
    w = win(:)';    % w is the window row vector
end
if (nargin < 3) || isempty(inc)
    inc = lw;       % if so, frame increasement in samples is the window length
end
nli=nx-lw+inc;      % speech length - window length + frame increasement
nf = fix((nli)/inc); % compute the number of frames
na=nli-inc*nf;      % zero pading length 
f=zeros(nf,lw);     % f is the output initilized with nf x lw zero matrix
indf= inc*(0:(nf-1)).';  
inds = (1:lw);
